...
async def stream_run(run_request: RunRequest, request: Request) -> AsyncIterator[str]:
    """Run a workflow and stream its only authoritative visual state."""

    run_id = f"run_{uuid4().hex}"
    sequence = RunEventSequence(run_id)
    yield encode_sse(
        RunStartedEvent(**sequence.fields(), client_run_id=run_request.client_run_id)
    )
    ...
    run_task = asyncio.create_task(
        scheduler.run(plan, run_input=run_request.input, cancellation=cancellation),
