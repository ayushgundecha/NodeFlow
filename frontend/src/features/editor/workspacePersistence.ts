...
const validWorkspace = (value: unknown): value is EditorWorkspace => {
  if (!isRecord(value) || value.format !== EDITOR_WORKSPACE_FORMAT || typeof value.id !== "string" || typeof value.name !== "string" || typeof value.description !== "string" || typeof value.savedAt !== "string" || !Array.isArray(value.nodes) || !Array.isArray(value.edges)) return false;
  const ids = new Set<string>();
  const nodesValid = value.nodes.every((node) => isRecord(node) && typeof node.id === "string" && !ids.has(node.id) && ids.add(node.id) && isRecord(node.position) && typeof node.position.x === "number" && typeof node.position.y === "number" && isRecord(node.data) && isNodeType(node.data.nodeType));
...
export function loadEditorWorkspace(storage: StorageLike = localStorage): WorkspaceLoadResult {
...
export function saveEditorWorkspace(workspace: EditorWorkspace, storage: Pick<Storage, "setItem"> = localStorage) {
  try { storage.setItem(EDITOR_WORKSPACE_STORAGE_KEY, serializeEditorWorkspace(workspace)); } catch { /* Storage failures must not make the editor unusable. */ }
}
